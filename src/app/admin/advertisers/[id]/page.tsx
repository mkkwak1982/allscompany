...
export default async function AdminAdvertiserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  await requireRole("ADMIN");
  const { id } = await params;

  const advertiser = await prisma.advertiser.findUnique({ where: { id }, include: { salesRep: true } });
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          사업자등록번호 {advertiser.businessRegNo} · 담당 영업자 {advertiser.salesRep.name} · 읽기 전용
        </p>
